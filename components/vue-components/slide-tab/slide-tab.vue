        ...
        <div class="slide-tab__bar__container" :style="containerStyles">
            <div class="slide-tab__bar__wrap">
                <div
                    ...
                    :class="['slide-tab__bar', isActive(index) && 'slide-tab__bar--active']"
                    @click="onSelectedIndexChange(index)"
                >
                    <div :class="['slide-tab__bar__text', isActive(index) && 'slide-tab__bar__text--active']">
                        {{ item.title }}
                    </div>
                </div>
...
class Page extends Vue {
    ...
    updateSelectedIndex(preIndex = 0, nextIndex = 0, duration = 0) {
        this.loaded[this.selectedIndex] = true
        this.contentWrapStyles = this.animation(-this.width * this.selectedIndex, duration)
    ...
    onTouchMove(event) {
        ...
        const translateX = parseInt(clientX - this.originX - this.width * this.selectedIndex, 10)
        if (this.lockX && this.canMove) {
