...
(function(window) {
  ...
  var browser = (function() {
     ...
     * @type {String}
     */
    var jsVendor = (data.jsVendor = data.webkit
      ? 'webkit'
      : data.firefox
        ? 'webkit'
        : data.opera
    ...
    if (supportTransform3D) {
      ...
      style.textContent =
        '@media (' + cssVendor + 'transform-3d){#test3d{height:3px}}'
  ...
  var Hilo = {
    ...
    copy: function(target, source, strict) {
      ...
      if (!hasWarnedDict.copy) {
        ...
        console.warn(
          'Hilo.copy has been Deprecated! Use Hilo.util.copy instead.'
        )
      }
      return target
    },
    ...
    getElementRect: function(elem) {
      ...
      var offsetX =
        (win.pageXOffset || docElem.scrollLeft) - (docElem.clientLeft || 0) || 0
      var offsetY =
        (win.pageYOffset || docElem.scrollTop) - (docElem.clientTop || 0) || 0
      ...
      var padLeft =
        parseIntFn(styles.paddingLeft) + parseIntFn(styles.borderLeftWidth) || 0
      var padTop =
        parseIntFn(styles.paddingTop) + parseIntFn(styles.borderTopWidth) || 0
      var padRight =
        parseIntFn(styles.paddingRight) + parseIntFn(styles.borderRightWidth) ||
        0
      var padBottom =
        parseIntFn(styles.paddingBottom) +
          parseIntFn(styles.borderBottomWidth) || 0
    ...
    setElementStyleByView: function(obj) {
      ...
      if (obj.transform) {
        ...
        if (
          ...
          style[prefix + 'TransformOrigin'] =
            obj.pivotX + px + ' ' + obj.pivotY + px
        }

        if (
          this.cacheStateIfChanged(
            obj,
    ...
    /** @lends Drawable.prototype */ {
      ...
      Statics: /** @lends Drawable */ {
        ...
        isDrawable: function(elem) {
          ...
          return (
            tagName === 'img' || tagName === 'canvas' || tagName === 'video'
          )
        }
      }
    }
...
;(function(window) {
  ...
  var Renderer = window.Hilo.Renderer
  ...
  var CanvasRenderer = Class.create(
    /** @lends CanvasRenderer.prototype */ {
      Extends: Renderer,
      constructor: function(properties) {
  ...
  var DOMRenderer = (function() {
    return Class.create({
      ...
      startDraw: function(target) {
        ...
        drawable.domElement =
          drawable.domElement || createDOMDrawable(target, drawable)
        return true
      },
    ...
    /** @lends WebGLRenderer.prototype */ {
      ...
      draw: function(target) {
        ...
        if (image) {
          ...
          var vertexs = this._createVertexs(
            image,
            rect[0],
            rect[1],
            sw,
          ...
          var tint =
            ...
            (target.tint & 0xff00) +
            ((target.tint & 0xff) << 16) +
            ((target.__webglRenderAlpha * 255) << 24)
          ...
          for (var i = 0; i < 4; i++) {
            ...
            float32Array[index + i * 5] =
              matrix.a * x + matrix.c * y + matrix.tx
            float32Array[index + i * 5 + 1] =
      ...
      transform: function(target) {
        ...
        if (target === this.stage) {
          ...
          target.__webglWorldMatrix =
            target.__webglWorldMatrix || new Matrix(1, 0, 0, 1, 0, 0)
        ...
        if (target.alpha > 0) {
          if (target.parent && target.parent.__webglRenderAlpha) {
            target.__webglRenderAlpha =
              target.alpha * target.parent.__webglRenderAlpha
          } else {
      ...
      resize: function(width, height) {
        if (this.width !== width || this.height !== height) {
          ...
          this.canvasHalfWidth = width * 0.5
          this.canvasHalfHeight = height * 0.5
      ...
      _renderBatches: function() {
        ...
        gl.bufferSubData(
          gl.ARRAY_BUFFER,
          0,
          this.uint32Array.subarray(0, this.batchIndex * this.positionStride)
        )
        var startIndex = 0
        var batchNum = 0
      ...
      _uploadProjectionTransform: function(force) {
        if (!this._projectionTransformElements || force) {
          this._projectionTransformElements = new Float32Array([
            1 / this.canvasHalfWidth,
            0,
            0,
            0,
            -1 / this.canvasHalfHeight,
            0,
            -1,
            1,
            1
        ...
        this.gl.uniformMatrix3fv(
          ...
          false,
          this._projectionTransformElements
        )
      },
      ...
      _createVertexs: function(img, tx, ty, tw, th, x, y, w, h) {
        ...
        tempVertexs[index++] = x
        tempVertexs[index++] = y
        tempVertexs[index++] = tx
        tempVertexs[index++] = ty
        tempVertexs[index++] = x + w
        tempVertexs[index++] = y
        tempVertexs[index++] = tx + tw
        tempVertexs[index++] = ty
        tempVertexs[index++] = x
        tempVertexs[index++] = y + h
        tempVertexs[index++] = tx
        tempVertexs[index++] = ty + th
        tempVertexs[index++] = x + w
        tempVertexs[index++] = y + h
        tempVertexs[index++] = tx + tw
        tempVertexs[index++] = ty + th
    ...
    return Class.create(
      /** @lends View.prototype */ {
        ...
        _render: function(renderer, delta) {
          if (
            (!this.onUpdate || this.onUpdate(delta) !== false) &&
      ...
      getViewAtPoint: function(x, y, usePolyCollision, global, eventMode) {
        ...
        for (var i = children.length - 1; i >= 0; i--) {
          ...
          if (
            ...
            !child.visible ||
            child.alpha <= 0 ||
            (eventMode && !child.pointerEnabled)
          )
            continue
          ...
          if (
            ...
            !(eventMode && !child.pointerChildren)
          ) {
            obj = child.getViewAtPoint(
              x,
              y,
              usePolyCollision,
              global,
    ...
    /** @lends Stage.prototype */ {
      ...
      _initRenderer: function(properties) {
        ...
        if (typeof container === 'string')
          container = Hilo.getElement(container)
      ...
      _onDOMEvent: function(e) {
        ...
        if (
          target &&
          ((target != obj && (!target.contains || !target.contains(obj))) ||
            leave)
        ) {
          var out =
            type === 'touchmove'
              ? 'touchout'
              : type === 'mousemove' || leave || !obj
                ? 'mouseout'
                : null
          if (out) {
        ...
        if (!isTouch) {
          var cursor =
            obj && obj.pointerEnabled && obj.useHandCursor ? 'pointer' : ''
  ...
  var Sprite = Class.create(
    /** @lends Sprite.prototype */ {
      ...
      _nextFrame: function(delta) {
        ...
        var value =
          frameIndex == 0 && !this.drawable
            ? 0
            : elapsed + (this.timeBased ? delta : 1)
  ...
  var DOMElement = Class.create(
    /** @lends DOMElement.prototype */ {
      ...
      constructor: function(properties) {
        ...
        this.drawable = new Drawable()
        var elem = (this.drawable.domElement =
          properties.element || Hilo.createElement('div'))
        elem.id = this.id
      ...
      render: function(renderer, delta) {
        if (renderer.renderType !== 'dom') {
          ...
          if (!renderer._domElementContainer) {
            domElementContainer = renderer._domElementContainer = Hilo.createElement(
              'div',
              {
                style: {
                  position: 'absolute',
                  transform:
                    'scale3d(' + stage.scaleX + ',' + stage.scaleY + ', 1)',
                  transformOrigin: '0 0',
                  zIndex: '1'
                }
              }
          ...
          var elem = this.drawable.domElement,
            depth = this.depth,
            nextElement = domElementContainer.childNodes[0],
            nextDepth
    ...
    return Class.create(
      /** @lends Graphics.prototype */ {
        ...
        lineStyle: function(
          ...
          var me = this,
            addAction = me._addAction

          addAction.call(me, ['lineWidth', (me.lineWidth = thickness || 1)])
          addAction.call(me, [
            'strokeStyle',
            (me.strokeStyle = lineColor || '0')
          ])
          addAction.call(me, ['lineAlpha', (me.lineAlpha = lineAlpha || 1)])
          if (lineCap != undefined)
            addAction.call(me, ['lineCap', (me.lineCap = lineCap)])
          if (lineJoin != undefined)
            addAction.call(me, ['lineJoin', (me.lineJoin = lineJoin)])
          if (miterLimit != undefined)
            addAction.call(me, ['miterLimit', (me.miterLimit = miterLimit)])
          me.hasStroke = true
          return me
        },
        ...
        beginLinearGradientFill: function(x0, y0, x1, y1, colors, ratios) {
          var me = this,
            gradient = helpContext.createLinearGradient(x0, y0, x1, y1)
        ...
        beginRadialGradientFill: function(
          ...
          var me = this,
            gradient = helpContext.createRadialGradient(x0, y0, r0, x1, y1, r1)
          for (var i = 0, len = colors.length; i < len; i++) {
            gradient.addColorStop(ratios[i], colors[i])
          }
          me.hasFill = true
        ...
        beginBitmapFill: function(image, repetition) {
          var me = this,
            pattern = helpContext.createPattern(image, repetition || '')
          me.hasFill = true
          ...
          for (var i = 0, len = path.length; i < len; i++) {
            ...
            case 'S':
              ...
              if (lastCmd === 'C' || lastCmd === 'S') {
                controlPoint = this._getReflectionPoint(
                  currentPoint,
                  lastControlPoint
                )
              } else {
...
;(function(window) {
   ...
   */
  var BitmapText = Class.create(
    /** @lends BitmapText.prototype */ {
      Extends: Container,
      constructor: function(properties) {
        properties = properties || {}
              ...
              if (typeof frames === 'string') {
                ...
                for (var j = 0, jl = all.length; j < jl; j++) {
                  ...
                  if (temp.length == 1) {
                    ...
                    for (
                      i = parseInt(temp[0]), l = parseInt(temp[1]);
                      i <= l;
                      i++
                    ) {
                      frames.push(i)
      ...
      for (var s in spriteData) {
        ...
        if (isNumber(sprite)) {
          ...
          for (i = sprite.from; i <= sprite.to; i++) {
            spriteFrames[i - sprite.from] = translateSpriteFrame(
              frames[i],
              sprite[i]
            )
          }
      ...
      /** @lends Tween.prototype */ {
        ...
        link: function(tween) {
          ...
          if (typeof delay === 'string') {
            ...
            delay =
              plus || minus
                ? Number(delay.substr(1)) * (plus ? 1 : -1)
                : Number(delay)
          }
          tween.delay = delay
          tween._startTime =
            plus || minus ? startTime + me.duration + delay : startTime + delay
        ...
        _update: function(time, forceUpdate) {
          ...
          if (me.reverse && me.isStart) {
            ...
            if (ratio < 1e-7) {
              ...
              if (
                (me.repeat > 0 && me._repeatCount++ >= me.repeat) ||
                (me.repeat == 0 && !me.loop)
              ) {
                me.isComplete = true
              } else {
          ...
          if (next && next.time <= 0) {
            ...
            if (nextStartTime > 0 && nextStartTime <= time) {
              ...
              me.isComplete &&
              (nextStartTime < 0 || nextStartTime > time)
            ) {
              //next tween
              next.start()
            }
...
;(function(window) {
  ...
  var Ease = (function() {
    ...
    var Elastic = createEase(
      ...
      function(k) {
        return -(
          Elastic.a *
          pow(2, 10 * (k -= 1)) *
          sin(((k - Elastic.s) * (2 * PI)) / Elastic.p)
        )
      },
    ...
    var Back = createEase(
      ...
      function(k) {
        return (k *= 2) < 1
          ? 0.5 * (k * k * ((Back.s + 1) * k - Back.s))
          : 0.5 * ((k -= 2) * k * ((Back.s + 1) * k + Back.s) + 2)
      }
    )
    ...
    var Bounce = createEase(
      ...
      function(k) {
        return k < 0.5
          ? Bounce.EaseIn(k * 2) * 0.5
          : Bounce.EaseOut(k * 2 - 1) * 0.5 + 0.5
      }
    )
  ...
  var ScriptLoader = Class.create({
    load: function(data) {
      ...
      if (isJSONP) {
        ...
        src +=
          (src.indexOf('?') == -1 ? '?' : '&') + callbackName + '=' + callback
      }

      if (data.noCache)
        src += (src.indexOf('?') == -1 ? '?' : '&') + 't=' + +new Date()
    ...
    /** @lends Camera.prototype */ {
      ...
      tick: function(deltaTime) {
        ...
        if (target) {
          ...
          if (deadzone) {
            viewX = Math.min(
              Math.max(target.x - scroll.x, deadzone[0]),
              deadzone[0] + deadzone[2]
            )
            viewY = Math.min(
              Math.max(target.y - scroll.y, deadzone[1]),
              deadzone[1] + deadzone[3]
            )
          } else {
            viewX = this.width * 0.5
          ...
          if (bounds) {
            scroll.x = Math.min(
              Math.max(scroll.x, bounds[0]),
              bounds[0] + bounds[2]
            )
            scroll.y = Math.min(
              Math.max(scroll.y, bounds[1]),
              bounds[1] + bounds[3]
            )
          }
        } else {
...
;(function(window) {
  ...
  var ParticleSystem = (function() {
    ...
    var ParticleSystem = Class.create(
      /** @lends ParticleSystem.prototype */ {
        Extends: Container,
        constructor: function(properties) {
