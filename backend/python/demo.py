...
def hello_world():  # 4、利用def关键字创建函数，简单来说：函数就是将具有独立功能的代码块组织成一个模块，需要的时候调用
    ...
    print(
        "欢迎来到Python的世界,", yourname
    )  # 6、输出函数,print()，若要输出多个对象，利用逗号分隔
    print("让我们开始学习吧~")
...
def deviding_line():
    ...
    endReturn = (
        line + words[random.randint(0, 3)] + line
    )  # 14、字符串可以利用+号直接相连
...
def study_number():
    num1 = input("请输入一个数字:")
    print(
        "你输入的是数字%s" % num1, "可它的类型为：", type(num1)
    )  # 17、输出函数格式控制
    # 18、type()函数可以返回该值的类型
    num2 = int(
        input("再输入一个数字:")
    )  # 19、利用int()函数进行数值类型转换，将数字转换为int整型
    ...
    )  # 24、num1*num2并不会是你想要的数据，因为input()函数，默认输入为字符类型
    print("num1*num2={}".format(int(num1) * num2))  # 25、数字乘法
    print(
        "num2//num3={:.3f}".format(num2 // num3)
    )  # 26、数字整除，同时{:.3f}表示输出格式小数点后面保留三位
    print("num2/num3={:.4f}".format(num2 / num3))  # 27、数字除法，保留小数点后四位
    print("num2%num3={:.4f}".format(num2 % num3))  # 28、求余数
    print(
        "num2%num3={:.4%}".format(num2 % num3)
    )  # 29、求余数，{:.4%}输出格式为百分比格式
    print("num1**num2={}".format(int(num1) ** num2))  # 30、幂运算
    print(
        "This is the {a},and {b}".format(a="numbers", b="some operations")
    )  # 31、format多参数，标记位置对应输出
...
def study_list(length):  # 36、带有参数的函数
    ...
    print(
        id(l1), id(l2), id(l3)
    )  # 42、id()函数可以获取对象的内存地址，在这里可以看到l3的的地址和l2是一样的
    l3[0] = 99  # 43、更新列表值
    print(
        "l2==l3么?", l2 == l3
    )  # 44、更新l3后依旧等于l2，因为l3和l2本来就是一个对象，不过换了个名字
...
def study_tuple(length: int) -> bool:  # 52、解释参数类型的函数创建，->为返回值类型
    ...
    try:
        ...
        tuple3 = (
            tuple1 + tuple2
        )  # 60、元组虽然不可改变，但是可以通过+号进行合并为另一个元组
        print(tuple3, id(tuple3))
        return True


# 学习python中的字典模型
def study_dict():  # 61、以下为创建字典的5种方法
    ...
    print(
        dict1[1], dict2["one"], dict3[6], dict4["One"], dict5[1]
    )  # 63、通过字典的键访问
    ...
    print(dict1)
    print(
        1 in dict1, 6 in dict1, 7 not in dict1
    )  # 67、in和not in关键字，可以判断值是否在序列中
    ...
    del (
        ...
        dict5,
        dict6,
    )  # 70、删除字典,也可以用del dict[key]的方式删除某个键
...
def study_set():
    ...
    set2 = {
        ...
        "So",
        "Beautiful",
    }  # 72、利用{}创建集合，创建空集合的时候不能用{},因为{}表示字典
    ...
    print(
        set1 <= set2, set3 <= set2, set3 < set2
    )  # 78、<=符号，判断是否为子集，<符号，判断是否为真子集
    set1.add("Me too")  # 79、集合添加元素
    print(
        "is语句用法", set3 == set2, set3 is set2, set1 is not set2
    )  # 80、is和is not语句，is语句用于判断对象是否一样，==判断值是否一样
...
def study_Some_functions():
    ...
    print(
        max(list1), max(tuple1), max(set1), max(dict1)
    )  # 82、max()函数，得到序列中最大值
    ...
    for i in range(len(list1)):  # 90、for循环语句
        print(
            list1[i], end=" "
        )  # 91、print的属性end，可以使输出格式为end的内容，而不是默认换行
    print()
    ...
    print("\n", list5)  # 94、\n,换行符
    testStr = (
        "The mountains and rivers are different, the wind and the moon are the same"
    )
    words = testStr.split(
        " "
    )  # 95、split()函数，以split()内参数分割字符串，返回一个列表
    print(words)
    words.sort(
        key=len
    )  # 96、sort()函数，进行排序，参数key=len时，以字符串长度为标准排序
    print("以长度排序:", words)
    ...
    ct = Counter(
        testStr
    )  # 99、collections模块中的Counter,可以得到字符串中每个数字出现次数
    print(ct)
...
def study_Slice():
    ...
    print(
        str1[:]
    )  # 102、切片格式为str[start:end:step]，前闭后开,step可为正负，默认步长为1
...
def study_loop_select():
    ...
    while i <= num:  # 108、while expression:当expression为真的时候进行循环
        if (
            i < 5
        ):  # 109、if...elif...else选择语句，如果判断结果只有两个,可以使用if...else
            print("我打印了", i, "次")
        elif i < 10:
            print("打印了", i, "次，真累啊")
        elif i < 15:
        ...
        time.sleep(
            0.5
        )  # 112、time库为python中的时间库，time.sleep(second)可以使程序暂停运行second秒
    else:  # 113、while循环后接一个else语句，当执行完所有循环后执行一次，可以省略(个人感觉用处不大)
        print("while结束了")


def study_expression_deduction():
    list1 = [i for i in range(10)]  # 114、python表达式推导
    list2 = [
        x for x in range(20) if x % 2 == 0
    ]  # 115、语句中增加if，满足if后表达式的才会是列表
    ...
    suitcolor = [
        (color, fruit) for color, fruit in zip(colors, fruits)
    ]  # 116、两个列表合并
    print(suitcolor)
    cartesian = [
    ...
    print(cartesian)
    dict1 = {
        fruit: color for fruit, color in suitcolor
    }  # 118、字典的推导，只要是带有键值对的任何序列，都可以推导出字典
    print(dict1)
...
def study_files():
    ...
    if filepath == "quit":
        return True
    try:
        file = open(filepath, "w")  # 119、打开文件，'w'为写格式打开
        file.write("哈哈，现在开始写文件")  # 120、向文件写入字符串
        file.close()  # 121、关闭文件
...
class Users:  # 125、面向对象编程，python中创建类class，类包含有属性与方法，包括有私有变量，共有变量等等
    ...
    def display(self):  # 127、类方法
        print(
            "大家好，我是{},身高{},体重{},颜值超高{}".format(
                self.name, self.height, self.weight, self.yanzhi
            )
        )


if __name__ == "__main__":  # 128、无论之前有什么，程序都会从这里开始运行
    hello_world()  # 129、运行的第一句，调用该函数
    deviding_line()
    try:
        print(yourname)  # 130、尝试输出hello_world()中的局部变量
    except:
        print("  未能找见该变量  ")  # 131、预期中的错误处理
    deviding_line()
    hello_twice()  # 132、定义全局变量
    user = Users(yourname, yourheight, yourweight)  # 133、实例化对象
    user.display()  # 134、调用对象方法
    ...
    while True:  # 135、主循环
        ...
        if num == "quit":
            ...
            study_files()
        deviding_line()
    print("哈哈，恭喜你，这个程序结束咯~")  # 137、程序结束语
